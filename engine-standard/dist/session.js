...
class SessionManager {
                    ...
                    if (!foundLid && !jid.includes("@lid")) {
                        try {
                            ...
                            const query = new baileys_1.USyncQuery()
                                .withMode("query")
                                .withUser(new baileys_1.USyncUser().withPhone(payload.number))
                                .withLIDProtocol();
    ...
    async startSession(payload, tenantId) {
        ...
        try {
            ...
            const { state, saveCreds } = await (0, baileys_1.useMultiFileAuthState)(path_1.default.join(this.sessionsDir, `session-${payload.sessionId}`));
            ...
            sock.ev.on("connection.update", async (update) => {
                ...
                if (connection === "close") {
                    ...
                    const noRetryCodes = [
                        baileys_1.DisconnectReason.loggedOut, // 401 - User logged out
                        baileys_1.DisconnectReason.forbidden, // 403 - Banned
                        baileys_1.DisconnectReason.badSession // 500 - Auth file corrupted
                    ];
                    ...
                    if (payload.usePairingCode && !sock.authState.creds.registered && statusCode === baileys_1.DisconnectReason.loggedOut) {
                    ...
                    if (shouldReconnect && currentRetries < 5) {
                        ...
                        logger_1.logger.error(`Session ${payload.sessionId} failed to connect after ${currentRetries} attempts or permanent error (code: ${statusCode}).`);
                        this.retries.delete(payload.sessionId);
                        this.sessions.delete(payload.sessionId);
                        const statusEvent = {
                            id: (0, uuid_1.v4)(),
    ...
    async sendText(payload) {
        ...
        logger_1.logger.info(`[sendText] Sending text to ${payload.to}: ${payload.body}`);
        const msg = await session.socket.sendMessage(payload.to, {
            text: payload.body
        });
        if (msg) {
            logger_1.logger.info(`[sendText] Message sent, triggering handleMessage. Msg ID: ${msg.key.id} Tenant: ${session.tenantId}`);
            await this.handleMessage(msg, session.socket, payload.sessionId, session.tenantId);
        }
        else {
            logger_1.logger.warn(`[sendText] Message rejected or no response from Baileys.`);
        }
    }
    async handleMessage(msg, sock, sessionId, tenantId) {
        if (!msg.message)
            return;
    ...
    async sendMedia(payload, tenantId) {
        ...
        logger_1.logger.info(`Sending media to ${payload.to}: ${payload.media.filename}`);
    ...
    async sendCarousel(payload) {
        ...
        const cards = await Promise.all(payload.cards.map(async (card) => {
            ...
            if (card.headerUrl) {
                ...
                const media = await (0, baileys_1.prepareWAMessageMedia)({ image: { url: card.headerUrl } }, { upload: session.socket.waUploadToServer });
                cardObj.header = {
                    hasMediaAttachment: true,
                    ...media
                };
        ...
        const msg = (0, baileys_1.generateWAMessageFromContent)(payload.to, messageContent, {
