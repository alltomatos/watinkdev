...
export default function PermissionTransferList({ allPermissions = [], selectedPermissions = [], onChange }) {
    ...
    <Card className={classes.paper} elevation={0}>
      ...
      <List className={classes.list} dense component="div" role="list">
        ...
        {items.map((value) => {
          ...
          const displayText = value.name || value.description || `${value.resource}:${value.action}` || `ID: ${value.id}`;
          const secondaryText = value.name && value.description ? value.description : (value.resource ? `${value.resource}:${value.action}` : null);
  ...
  return (
    <Grid container spacing={2} className={classes.root}>
      <Grid item xs={12} md={5}>{customList(i18n.t("role.permissions.available") || "Disponíveis", filteredLeft, searchLeft, setSearchLeft)}</Grid>
      ...
      <Grid item xs={12} md={5}>{customList(i18n.t("role.permissions.assigned") || "Atribuídas", filteredRight, searchRight, setSearchRight)}</Grid>
    </Grid>
  );
}
