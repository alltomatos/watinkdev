...
module.exports = {
    up: async (queryInterface) => {
        // 1. Find admin user
        const users = await queryInterface.sequelize.query(`SELECT id, "tenantId" FROM "Users" WHERE email = 'admin@admin.com' LIMIT 1;`);
        if (users[0].length === 0) {
        ...
        let groupId;
        const groups = await queryInterface.sequelize.query(`SELECT id FROM "Groups" WHERE name = 'Admin' AND "tenantId" = '${tenantId}' LIMIT 1;`);
        if (groups[0].length > 0) {
            ...
            const newGroups = await queryInterface.sequelize.query(`SELECT id FROM "Groups" WHERE name = 'Admin' AND "tenantId" = '${tenantId}' LIMIT 1;`);
            groupId = newGroups[0][0].id;
        }
        // 4. Update User to belong to this Group
        await queryInterface.sequelize.query(`UPDATE "Users" SET "groupId" = ${groupId} WHERE id = ${user.id}`);
        ...
        const existingGroupPermissions = await queryInterface.sequelize.query(`SELECT "permissionId" FROM "GroupPermissions" WHERE "groupId" = ${groupId}`);
