...
class AIController {
    ...
    async search(req, res) {
        ...
        try {
            const results = await EmbeddingService_1.default.searchSimilar(query, tenantId, limit, contactId);
     ...
     */
    async getContactInsights(req, res) {
        const { tenantId } = req.user;
        const { id } = req.params;
        ...
        try {
            const insights = await EmbeddingService_1.default.getContactInsights(parseInt(id, 10), tenantId);
            if (!insights) {
                return res.json({
    ...
    async processTicket(req, res) {
        ...
        try {
            const embedding = await EmbeddingService_1.default.processTicket(parseInt(id, 10), tenantId);
            if (!embedding) {
    ...
    async ask(req, res) {
        ...
        try {
            ...
            const relevantConversations = await EmbeddingService_1.default.searchSimilar(question, tenantId, 3, contactId);
            // Build context from relevant conversations
            const context = relevantConversations
                .map(c => `Conversa (Ticket #${c.ticketId}): ${c.summary}`)
                .join("\n\n");
            // Call AI for answer
            const config = await EmbeddingService_1.default.getConfig(tenantId);
