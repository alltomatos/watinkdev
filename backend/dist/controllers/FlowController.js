...
const generateFlowAI = async (req, res) => {
    ...
    const flowData = await FlowAIService_1.default.generateFlowFromPrompt(prompt, tenantId);
...
const store = async (req, res) => {
    ...
    const flow = await (0, FlowService_1.CreateFlowService)({
        name,
        nodes,
        edges,
        tenantId,
...
const index = async (req, res) => {
    const { tenantId } = req.user;
    const flows = await (0, FlowService_1.ListFlowsService)({ tenantId });
...
const update = async (req, res) => {
    ...
    const { tenantId } = req.user;
    const flow = await (0, FlowService_1.UpdateFlowService)({
        id: Number(flowId),
        flowData,
        tenantId
    });
...
const toggle = async (req, res) => {
    const { flowId } = req.params;
    const { tenantId } = req.user;
    const flow = await (0, FlowService_1.ToggleFlowService)({
        id: Number(flowId),
        tenantId
    });
    return res.json({
...
const simulate = async (req, res) => {
    ...
    // Executar simulação
    const result = await FlowExecutorService_1.default.simulateFlow(flow, message || "Olá, teste de simulação");
