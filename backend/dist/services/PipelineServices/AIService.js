...
const AIService = (history, tenantId) => __awaiter(void 0, void 0, void 0, function* () {
    // Buscar API Key nas configurações do tenant
    const apiKeySetting = yield Setting_1.default.findOne({
        where: { key: "aiApiKey", tenantId }
    });
    const providerSetting = yield Setting_1.default.findOne({
        where: { key: "aiProvider", tenantId }
    });
    const modelSetting = yield Setting_1.default.findOne({
    ...
    if (!apiKey) {
        ...
        throw new AppError_1.default("ERR_NO_AI_API_KEY");
    }
    ...
    const guidePromptSetting = yield Setting_1.default.findOne({
    ...
    `;
    try {
        const response = yield fetch(`${baseURL}/chat/completions`, {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new AppError_1.default(detailedError);
        }
        ...
        throw new AppError_1.default(`AI Service Failed: ${err.message}`);
    }
});
