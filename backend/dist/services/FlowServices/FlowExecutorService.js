...
class FlowExecutorService {
    start(flowId, context) {
        return __awaiter(this, void 0, void 0, function* () {
            const flow = yield Flow_1.default.findByPk(flowId);
            if (!flow)
                throw new AppError_1.default("Flow not found", 404);
            const nodes = flow.nodes;
            // 1. Create Session
            const session = yield FlowSession_1.default.create({
                flowId,
                status: "active",
                context,
            ...
            if (!startNode) {
                yield session.update({ status: "failed" });
    ...
    next(sessionId, input) {
        return __awaiter(this, void 0, void 0, function* () {
            const session = yield FlowSession_1.default.findByPk(sessionId);
            if (!session || session.status !== "active") {
                throw new AppError_1.default("Session not active", 400);
            }
            const flow = yield Flow_1.default.findByPk(session.flowId);
            if (!flow)
                throw new AppError_1.default("Flow not found", 404);
            const nodes = flow.nodes;
            ...
            const newContext = Object.assign(Object.assign({}, session.context), { lastInput: input });
            yield session.update({ context: newContext });
            ...
            if (nextEdges.length === 0) {
                yield session.update({ status: "completed", currentStepId: null });
                return session;
            }
            ...
            try {
                switch (node.type) {
                    ...
                    case "textUpdater": // "Pergunta"
                        ...
                        yield this.sendMessage(session, node.data.content || node.data.label || '', node.data);
                    ...
                    case "menu":
                        yield this.sendMenu(session, node.data);
                        return session; // Stop and wait for user reply
                    case "knowledge":
                        // Integração com RAG (VectorService)
                        const knowledgeQuery = session.context.lastInput || "";
                        yield this.processKnowledgeNode(session, node.data, knowledgeQuery);
                        return this.proceedToNext(session, flow, node);
                    // Integração Kanban/CRM
                    case "pipeline":
                        yield this.processKanbanAction(session, node.data);
                        return this.proceedToNext(session, flow, node);
                    case "ticket":
                        yield this.updateTicket(session, node.data);
                        return this.proceedToNext(session, flow, node);
                    case "webhook":
                        yield this.processWebhookNode(session, node.data);
                        return this.proceedToNext(session, flow, node);
                    case "switch":
                    ...
                    case "api":
                        yield this.processAPINode(session, node.data);
                        return this.proceedToNext(session, flow, node);
                    case "helpdesk":
                        yield this.processHelpdeskNode(session, node.data);
                        return this.proceedToNext(session, flow, node);
                    default:
    ...
    proceedToNext(session, flow, currentNode, sourceHandle) {
        return __awaiter(this, void 0, void 0, function* () {
            const nodes = flow.nodes;
            const edges = flow.edges;
    ...
    sendMessage(session, text, nodeData) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (contentType === 'text') {
                ...
                try {
                    ...
                    const mediaFile = yield this.downloadMedia(processedUrl, Number(ticket.tenantId));
                    if (mediaFile) {
    ...
    sendMenu(session, nodeData) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (options.length === 0) {
                yield this.sendMessage(session, text);
                return;
            }
            // Determine Buttons vs List
            if (options.length <= 3) {
                yield (0, SendWhatsAppInteractive_1.default)({
                    body: text,
                    ticket,
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (resultVariable) {
                    ...
                    // Let's store data.
                    const newContext = Object.assign(Object.assign({}, context), { [resultVariable]: response.data });
                    yield session.update({ context: newContext });
                ...
                if (resultVariable) {
                    const newContext = Object.assign(Object.assign({}, context), { [resultVariable]: { error: true, message: err.message, status: (_a = err.response) === null || _a === void 0 ? void 0 : _a.status } });
                    yield session.update({ context: newContext });
                }
            }
        });
    }
    processWebhookNode(session, nodeData) {
        return __awaiter(this, void 0, void 0, function* () {
            const context = session.context;
            ...
            try {
                ...
                if (nodeData.fullData && context.ticketId) {
                    const ticket = yield (0, ShowTicketService_1.default)(context.ticketId);
                    if (ticket) {
                        parsedBody = Object.assign(Object.assign({}, parsedBody), { ticket });
                        if (ticket.contact) {
                            parsedBody = Object.assign(Object.assign({}, parsedBody), { contact: ticket.contact });
                        }
                        try {
                            ...
                            if (deal)
                                parsedBody = Object.assign(Object.assign({}, parsedBody), { pipeline: deal });
                        }
                        catch (err) { }
                    }
                }
                    ...
                    if (nodeData.contactFields && nodeData.contactFields.length > 0 && context.ticketId) {
                        ...
                        if (ticket && ticket.contact) {
                            ...
                            parsedBody = Object.assign(Object.assign({}, parsedBody), { contact: filteredContact });
                        }
                    }
                    ...
                    if (nodeData.ticketFields && nodeData.ticketFields.length > 0 && context.ticketId) {
                        ...
                        parsedBody = Object.assign(Object.assign({}, parsedBody), { ticket: filteredTicket });
                    }
                    ...
                    if (nodeData.pipelineFields && nodeData.pipelineFields.length > 0 && context.ticketId) {
                        try {
                            ...
                            if (deal) {
                                ...
                                parsedBody = Object.assign(Object.assign({}, parsedBody), { pipeline: filteredPipeline });
                            }
                        }
                        catch (err) {
                ...
                if (nodeData.includeContext) {
                    parsedBody = Object.assign(Object.assign({}, parsedBody), { context });
                }
    ...
    processKanbanAction(session, nodeData) {
        return __awaiter(this, void 0, void 0, function* () {
            const context = session.context;
            if (!context.ticketId)
                return;
            ...
            try {
                ...
                if (kanbanAction === 'createDeal') {
                    ...
                    const deal = yield Deal_1.default.findOne({ where: { ticketId: ticket.id } });
                    if (deal) {
                        yield deal.update({ stageId: parseInt(sId) });
    ...
    evaluateSwitch(session, flow, node) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    executeDatabase(session, flow, node) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!allowedTables.includes(tableName)) {
                ...
                yield this.sendMessage(session, `Erro: Tabela "${tableName}" não permitida.`);
                return this.proceedToNext(session, flow, node);
            }
            const Model = this.getModelByName(tableName);
            if (!Model) {
                logger_1.logger.error(`FlowExecutor: Unknown table ${tableName}`);
                yield this.sendMessage(session, `Erro: Tabela "${tableName}" não encontrada.`);
                return this.proceedToNext(session, flow, node);
            }
            let result = null;
            try {
                switch (operation) {
                    case 'read': {
                        ...
                        result = yield Model.findAll(queryOptions);
                    ...
                    case 'update': {
                        ...
                        if (Object.keys(where).length === 0) {
                            ...
                            yield this.sendMessage(session, `Erro: UPDATE sem filtro não é permitido.`);
                            return this.proceedToNext(session, flow, node);
                        }
                        ...
                        if (Array.isArray(dataFields) && dataFields.length > 0) {
                            ...
                            try {
                                ...
                                yield this.sendMessage(session, `Erro: JSON inválido para atualizar registro.`);
                                return this.proceedToNext(session, flow, node);
                            }
                        }
                        else {
                        ...
                        if (!updateData || Object.keys(updateData).length === 0) {
                            yield this.sendMessage(session, `Erro: Nenhum dado para atualizar.`);
                            return this.proceedToNext(session, flow, node);
                        }
                        const [affectedRows] = yield Model.update(updateData, { where });
                        result = { affectedRows };
                    ...
                    default:
                        ...
                        yield this.sendMessage(session, `Operação ${operation} não permitida no modo NOCODE.`);
                }
                ...
                if (result !== null) {
                    const newContext = Object.assign(Object.assign({}, context), { [sanitizedOutputVar]: Array.isArray(result)
                ...
                yield this.sendMessage(session, `Erro na operação de banco de dados: ${err.message}`);
            }
            return this.proceedToNext(session, flow, node);
        });
    }
    // NOCODE: Executor do Filter Node - filtra dados de uma variável
    executeFilter(session, flow, node) {
        return __awaiter(this, void 0, void 0, function* () {
            const nodeData = node.data;
            const context = session.context;
            ...
            if (!inputData) {
                ...
                const newContext = Object.assign(Object.assign({}, context), { [sanitizedOutput]: [] });
                yield session.update({ context: newContext });
                return this.proceedToNext(session, flow, node);
            }
            ...
            if (!filterConditions || filterConditions.length === 0) {
                const newContext = Object.assign(Object.assign({}, context), { [sanitizedOutput]: dataToFilter });
                yield session.update({ context: newContext });
            ...
            // Armazenar resultado
            const newContext = Object.assign(Object.assign({}, context), { [sanitizedOutput]: filteredData });
            yield session.update({ context: newContext });
    ...
    processKnowledgeNode(session, nodeData, query) {
        return __awaiter(this, void 0, void 0, function* () {
            const context = session.context;
            if (!context.ticketId)
                return;
            ...
            try {
                if (!query || query.trim().length === 0) {
                    yield this.sendMessage(session, "Desculpe, não recebi sua pergunta. Poderia reformular?");
                    return;
                }
                // Buscar chunks relevantes via VectorService
                const results = yield VectorService_1.default.similaritySearch(query, tenantId, 5);
                if (results.length === 0) {
                    yield this.sendMessage(session, "Não encontrei informações relevantes sobre sua pergunta. Gostaria de falar com um atendente?");
                    return;
                }
                ...
                if (responseMode === 'search') {
                    ...
                    yield this.sendMessage(session, `📚 *Informações encontradas:*\n\n${contextText.substring(0, 1000)}...`);
                }
                else {
                    ...
                    yield this.sendMessage(session, `Com base nas informações disponíveis:\n\n${contextText.substring(0, 1500)}`);
                }
                // Atualizar contexto com resultado
                const newContext = Object.assign(Object.assign({}, context), { knowledgeResult: contextText.substring(0, 500) });
                yield session.update({ context: newContext });
            }
            catch (err) {
                logger_1.logger.error(`FlowExecutor: Error in Knowledge Node: ${err}`);
                yield this.sendMessage(session, "Houve um erro ao consultar a base de conhecimento. Tente novamente.");
            }
        });
    }
    // Processamento do Nó de Helpdesk (Criar/Consultar Protocolo)
    processHelpdeskNode(session, nodeData) {
        return __awaiter(this, void 0, void 0, function* () {
            const context = session.context;
            if (!context.ticketId)
                return;
            ...
            try {
                if (action === 'createProtocol') {
                    ...
                    // Criar protocolo
                    const protocol = yield Protocol_1.default.create({
                        tenantId: tenantId,
                    ...
                    const newContext = Object.assign(Object.assign({}, context), { protocol: protocolNumber, protocolId: protocol.id });
                    yield session.update({ context: newContext });
                    // Mensagem de confirmação (se configurado)
                    if (nodeData.sendConfirmation !== false) {
                        yield this.sendMessage(session, `✅ Protocolo criado com sucesso!\n\n📋 *Número:* ${protocolNumber}`);
                    }
                }
                else if (action === 'checkStatus') {
                    // Buscar protocolo mais recente do contato
                    const existingProtocol = yield Protocol_1.default.findOne({
                        where: {
                            contactId: ticket.contactId,
                            tenantId: tenantId
                        },
                    ...
                    if (existingProtocol) {
                        ...
                        yield this.sendMessage(session, `📋 *Protocolo:* ${existingProtocol.protocolNumber}\n📊 *Status:* ${statusMap[existingProtocol.status] || existingProtocol.status}`);
                    }
                    else {
                        yield this.sendMessage(session, "Não encontrei protocolos anteriores associados ao seu contato.");
                    }
                }
            }
            catch (err) {
                logger_1.logger.error(`FlowExecutor: Error in Helpdesk Node: ${err}`);
                yield this.sendMessage(session, "Houve um erro ao processar sua solicitação de protocolo.");
            }
        });
    }
}
